...
// Phase "first" drives the full journey (configure, test, request, approve,
// execute). Phase "second" relaunches the SAME userData to prove restore:
// it asserts the prior Work Unit/outcome/evidence is rendered through the UI
// and deliberately does NOT rewrite the credential saved on the first launch.
...
async function main() {
  ...
  if (PHASE === 'second') {
    const setupVisible = (await setupButton.count()) > 0 && await setupButton.isVisible().catch(() => false)
    assert(!setupVisible, 'AAOP already installed on second launch (no setup button shown)')
    console.log('AAOP already present through the UI (setup persisted from first launch)')
  } else if (await setupButton.count()) {
  ...
  if (PHASE === 'second') {
    step('1c. prior work unit restored through the UI (second launch)')
    // The product restores the last persisted Work Unit from the same userData
    // and renders it in the UI (resume card + title/outcome/evidence). These
    // are DOM facts a human sees; they prove restore WITHOUT re-running any
    // provider configuration.
    await page.waitForSelector('#resume-card:not(.hidden)', { timeout: 30_000 })
    const resumeStatus = await page.locator('#resume-status').textContent().catch(() => '')
    const workTitle = await page.locator('#work-title').textContent().catch(() => '')
    const workOutcome = await page.locator('#work-outcome').textContent().catch(() => '')
    const workState = await page.locator('#work-state').textContent().catch(() => '')
    const workUnitId = await page.locator('#resume-workunit-id').textContent().catch(() => '')
    const evidenceItems = await page.locator('#evidence-list li').count().catch(() => 0)
    console.log(`resume status="${resumeStatus}" workTitle="${workTitle}" workOutcome="${workOutcome}" workState="${workState}" workUnitId="${workUnitId}" evidenceItems=${evidenceItems}`)
    assert(resumeStatus.length > 0, 'resume status rendered through the UI')
    assert(workTitle.trim().length > 0, 'prior work unit title restored through the UI')
    assert(workOutcome.trim().length > 0, 'prior work unit outcome restored through the UI')
    assert(workUnitId.trim().length > 0, 'prior work unit id restored through the UI')
    assert(evidenceItems > 0, 'prior work unit evidence restored through the UI')
  }
  ...
  if (PHASE === 'first') {
    ...
    step('3b. provider/execution skipped on second launch')
    console.log('second launch verifies restored state through the UI only; the credential saved on first launch is left untouched')
  }
